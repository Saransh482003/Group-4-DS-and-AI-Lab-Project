...
class NavigationDeterministicLogic:
	def __init__(
		self,
		frame_width,
	...
	def _decide_command(self, zone_risks):
		...
		# Priority: explicitly select left when it is clearly safer than both center and right.
		if (
		...
		# Relative selection rule: if center is worst, choose the lowest-risk zone if it is safe.
	...
	def draw_overlays(self, frame, zone_risks, command):
		...
		# Draw zone boundaries only (no filled shading).
